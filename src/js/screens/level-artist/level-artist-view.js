...
const getAnswerWrapperTemplate = (
  answerNumber,
  artistName,
  artistImage
...
const getScreenLevelArtistTemplate = (
  timerTemplate,
  mistakesNumber,
  question
) => {
  const answersTemplate = question.answerList
    .map((answer, answerIndex) =>
      getAnswerWrapperTemplate(answerIndex++, answer.artist, answer.image)
    )
    .join(``);
...
class LevelArtistView extends AbstractView {
  ...
  get template() {
    return getScreenLevelArtistTemplate(
  ...
  bind() {
    ...
    playButton.addEventListener(`click`, () =>
      this._playButtonClick(playButton)
    );
