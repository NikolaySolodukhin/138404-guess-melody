const getPlayerResult = (resultsOtherPlayers, resultCurrentPlayer) => {
  ...
  const currentPlayerIndex = playersStats.filter(
    playerStats => playerStats < resultCurrentPlayer.score
  ).length;
  ...
  const defeatedPlayersPercent = Math.trunc(
    (currentPlayerIndex / playersStats.length) * 100
  );
